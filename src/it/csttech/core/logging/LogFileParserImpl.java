...
public class LogFileParserImpl implements LogFileParser
{
    ...
    public LogFileParserImpl(String filename, String regex, String timestampFormat, Long currentPosition)
    {
		...
		} else if (currentPosition >= this.currentPosition) { // this.currentPosition ora coincide con beginningOfMessages.
     ...
     */
    private List<String> prevMessage(){ // TODO: avviare ipotesi di unificazione.
/*    	if (useRegisters) {
            if (currentMessage <= 1){
                return null;
            }
...
//		}
    ...
    // Resetta le posizioni tramite i registri, il risultato sarà avere il cursore all'inizio del messaggio corrispondente.
    ...
    private boolean arePositionsSet(int messageNumber){
    	if (messageNumber + 1 < messageInitPositions.size()){ //This happens if we have already stored all position info about the message referenced by the user; we exploit this
